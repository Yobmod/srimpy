...
def add_comments() -> None:
    ...
    for path_object in root.glob('**/*'):
        if path_object.is_file():
            if str(path_object).endswith(".py") and not str(path_object).startswith("dev"):
...
def yaml_to_toml_or_json() -> None:
    ...
    if len(sys.argv) < 3:
        raise Exception('Usage is `yml_to_toml.py input.yml output.toml`')
    ...
    with open(yml_file) as source:
        ...
        toml_str_unsorted = toml.dumps(json_dict).encode(
